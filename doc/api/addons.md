...
本文档的其余部分将重点介绍后一种方式，因此需要了解多个组件和 API：
...
使用 `NODE_MODULE()` 定义的插件无法同时在多个上下文或多个线程中加载。
...
* 成为一个 [Node-API][] 插件。
* 如上所述，使用 `NODE_MODULE_INIT()` 声明为感知上下文的插件。
...
以下是一些示例插件，旨在帮助开发者入门。这些示例使用 V8 API。有关各种 V8 调用的帮助，请参阅在线的 [V8 参考文档][v8-docs]，有关句柄、作用域、函数模板等概念的说明，请参阅 V8 的[嵌入器指南][]. 
...
在进程或工作线程关闭期间，JS 引擎不会调用析构函数。因此，用户有责任跟踪这些对象并确保它们得到正确析构，以避免资源泄漏。
...
除了包装并返回 C++ 对象之外，还可以通过使用 Node.js 辅助函数
`ObjectWrap::Unwrap` 解包包装对象，从而传递这些对象。以下示例展示了一个
`add()` 函数，该函数可以接收两个 `MyObject` 对象作为输入参数：
