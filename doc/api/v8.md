...
Stopping collecting the profile and then return a Promise that will be fulfilled with the error or the profile data.
...
V8 only supports `Latin-1/ISO-8859-1` and `UTF16` as the underlying representation of strings.
This function will return true if `content` uses `Latin-1/ISO-8859-1` as its underlying representation;
otherwise, it returns false.

If this method returns false, it does not mean that the string contains characters that are not in `Latin-1/ISO-8859-1`.
Sometimes `Latin-1` strings may also be represented as `UTF16`.
