...
* `options` {integer | Object}
  * `family` {integer|string} 记录家族。必须是 `4`、`6` 或 `0`。出于向后兼容原因，`'IPv4'` 和 `'IPv6'` 分别被解释为 `4` 和 `6`。值 `0` 表示返回 IPv4 或 IPv6 地址。如果值 `0` 与 `{ all: true }` 一起使用（见下文），则根据系统的 DNS 解析器，返回其中一个或两者 IPv4 和 IPv6 地址。**默认值：** `0`。
