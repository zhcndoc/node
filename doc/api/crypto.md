...
* `options` {Object} [`stream.transform` 选项][]
  * `plaintextLength` {number}
  * `encoding` {string} 当 `buffer` 是字符串时使用的字符串编码。
* 返回：{Decipheriv} 同一个 Decipher 用于方法链式调用。
...
* `parameters` {Object}
  ...
  * `nonce` {string|ArrayBuffer|Buffer|TypedArray|DataView} 必需，必须至少 8 字节长。这是 Argon2 密码哈希应用的盐。
...
提供异步 [Argon2][] 实现。Argon2 是一种基于密码的密钥派生函数，旨在计算和内存方面都很昂贵，以使暴力破解攻击无利可图。
...
提供同步 [Argon2][] 实现。Argon2 是一种基于密码的密钥派生函数，旨在计算和内存方面都很昂贵，以使暴力破解攻击无利可图。
...
* `offset` {number} **默认：** `0`
* `size` {number} **默认：** `buffer.length - offset`。`size` 不得大于 `2**31 - 1`。
* Returns: {ArrayBuffer|Buffer|TypedArray|DataView} 作为 `buffer` 参数传递的对象。
...
提供异步 [scrypt][] 实现。Scrypt 是一种基于密码的密钥派生函数，旨在计算和内存方面昂贵，以使暴力破解攻击无利可图。
...
提供同步 [scrypt][] 实现。Scrypt 是一种基于密码的密钥派生函数，旨在计算和内存方面昂贵，以使暴力破解攻击无利可图。
...
* Returns: {Object}
  ...
  * `utilization` {number} 计算的 `used` 与 `total` 分配字节的比率。
