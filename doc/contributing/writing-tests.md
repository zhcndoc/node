...
这是测试的主体。这个测试很简单，它只是测试一个 HTTP 服务器是否接受传入请求头中的 `非 ASCII` 字符。值得注意的几点：
...
曾经出现过这样的情况：测试在没有 `console.log()` 时失败，而添加它之后却通过了，因此应谨慎使用它，尤其是在测试 I/O 和流式 API 时。
...
无需在任何地方列出该文件：`configure.py` 会将 `test/cctest` 下的每个 `.cc`
和 `.h` 文件收集到 `node_cctest_sources` 变量中，该变量由 node.gyp 中的
`cctest` 目标进行构建。

如果测试只能在启用某项功能时构建，请将其添加到 node.gyp 中相应的变量，
以便在其他情况下将其排除在构建之外：

* `node_cctest_openssl_sources`：需要加密支持的测试
* `node_cctest_quic_sources`：需要 QUIC 支持的测试
* `node_cctest_inspector_sources`：需要检查器支持的测试

应放置在 `test/cctest` 中的源文件只能是真正的测试文件或辅助源文件。
有时可能需要包含由 `node` 目标编译的特定目标文件，此时可以将它们添加到
`cctest` 目标的 `libraries` 部分。
